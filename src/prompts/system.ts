...
NUNCA por "yo" ni "buscando... encontré". TODO en TÚ singular (saludo incluido: "Hola, NOMBRE:").
...
<p style="margin:0 0 10px 0">Si alguien busca ${sectorLabel} en {{CIUDAD}} en Google, encuentra a <b>{{COMPETIDOR_PRINCIPAL}}</b> y a ti no, porque no tienes web. ${isFem ? 'Esos clientes' : 'Esas llamadas'} se van a quien sí aparece.</p>
${exampleUrl ? `<p style="margin:0 0 10px 0">${isFem ? 'A una' : 'A un'} ${sectorLabel} de tu zona le hice una (<a href="https://${exampleUrl}">${exampleUrl}</a>) y dejó de perderlos. Contigo sería igual.</p>` : ''}
<p style="margin:0 0 10px 0">¿Te paso el ejemplo y te cuento cómo lo haría con el tuyo?</p>
...
técnico). Idea: que aparezcas en Google no es un marrón para ti; lo monto yo entero, tú no tocas
nada, y empiezas sin poner dinero por adelantado. En una semana podrías estar saliendo.
NO repitas "no sales / llaman a otro" del email 1. Cierra con CTA concreta: "¿Te lo enseño en 2 min?".

email3_body — FU2 (día 7) — ÁNGULO NUEVO: que lo VEA con sus ojos (prueba visual), no recontar
la historia del caso. Idea: en vez de explicártelo, mejor que lo veas: ${exampleUrl
  ? `aquí tienes el de ${isFem ? 'una' : 'un'} ${sectorLabel} de tu zona y más ejemplos`
  : `aquí tienes ejemplos de negocios locales como el tuyo`}. NO recuentes "le hice la web y ahora
las ${queRegalan} son suyas" (ya está en el email 1). Incluye el enlace (único sitio donde va):
